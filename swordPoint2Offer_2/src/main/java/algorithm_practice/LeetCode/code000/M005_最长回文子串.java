...
//        System.out.println(maxLcpsLength(str3));
    ...
    private int maxLcpsLength(String str) {
        ...
        //记录回文半径所能扫到最右位置。（maxRight = i+pArr[i]-1）
        int maxRight = -1;
        //当maxRight更新的时候，此时回文中心的位置
        int index = -1;
        for (int i = 0; i < length; i++) {
            ...
            //尝试扩展
            while (i + pArr[i] < length && i - pArr[i] >= 0 &&
        ...
        //因为MaxLen为带有'#'的回文半径，所以不带'#'回文长度也就为MaxLen-1
        return MaxLen - 1;
    }
}
